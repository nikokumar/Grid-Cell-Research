function [  ] = Pair_wise_correlations(Rat,L_D,speed_threshold,Groups,G )
% Correlations between ineter and intra module pair-wise correlations
% Following the torus paper:
% 1. Bin spike trains to 10ms resulotion, here it's 8.3ms
% 2. Smooth by convolving with a gaussian with sigma-50ms
% 3. Discard times below speed threshold of 3 cm/sec ?
...
if Groups==1 % if we plot scatter plot, we look only at the zero lag correaltions
    max_lag=0;
end
...
if cluster==1
    ...
    name = [Na,sprintf(':speed_threshold=%d,Groups=%d,G=%d.mat',speed_threshold,Groups,G)]; % name of file
...
%     CORR_intra(n,:) = xcorr(smoothed_ST(Intra_Pairs(n,1),:) , smoothed_ST(Intra_Pairs(n,2),:),lag_ind);
end
...
%     CORR_inter(n,:) = xcorr(smoothed_ST(Inter_Pairs(n,1),:) , smoothed_ST(Inter_Pairs(n,2),:),lag_ind);
end
%% Organizing Results
