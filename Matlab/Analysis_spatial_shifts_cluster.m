Rat=11; % what rat data do we analyze? 1 for, 11 for Bubble 2nd dataset, 2 for Roger, 3 for Coconut, 4 for Rolf

identical=0; % plot control (identical shifts to all) or shifts by modules (0 for modules-wise results , 1 for identical shift results)
...
for ld=L_D
    ...
    %% Iterate on results    
    Norms_mean_and_SEM = zeros(2,numel(spatial_shift)); % will contain <log(Z)> values and corresponding SEM for each spatial shit
    Error_mean_and_SEM = zeros(2,numel(spatial_shift)); % will contain MAE values and corresponding SEM for each spatial shit
    Radi_err_mean_and_SEM = zeros(2,numel(spatial_shift)); % will contain mean radi error values and corresponding SEM for each spatial shit
    
    shift_counter=1; % counter of spatial shifts
    for j=spatial_shift
        ...
        for i=tr % running on all trials of this spatial shift     
            %% Loading file
            if identical==0
                name=[Na,sprintf('_spatial_shift=%d_tr=%d.mat',j,i)]; % name of file
            elseif identical==1
                name=[Na,sprintf('_identical_spatial_shift=%d_tr=%d.mat',j,i)]; % name of file for identical shifts case
            end
        ...
        No = No./t_count; % Average vs time norms over trials of stochatic of shifts and dilution of spikes (have norms vs time)
        Eo = Eo./t_count; % Average vs time mae over trials of stochatic of shifts and dilution of spikes (have mae vs time)
        Ro = Ro./t_count; % Average vs time radi error over trials of stochatic of shifts and dilution of spikes (have radi error vs time)
        
        
        Norms_mean_and_SEM(1,shift_counter) = mean(No); % <log(Z)> for the j'th spatial shift
        Norms_mean_and_SEM(2,shift_counter) = SEM_time_series(No,per_autocorr_decrease); % corresponding SEM
        
        Error_mean_and_SEM(1,shift_counter) = mean(Eo); % MAE for the j'th spatial shift
        Error_mean_and_SEM(2,shift_counter) = SEM_time_series(Eo,per_autocorr_decrease); % corresponding SEM
        
        Radi_err_mean_and_SEM(1,shift_counter) = mean(Ro); % mean radi error for the j'th spatial shift
        Radi_err_mean_and_SEM(2,shift_counter) = SEM_time_series(Ro,per_autocorr_decrease); % corresponding SEM
